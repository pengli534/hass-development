...
/*Pin definitions*/
const int programButton = 4;   // (Digital 0) Record A New Knock button.
...
const int lockPin = 6;         // (Digital 4) The pin that activates the relay/solenoid lock.
...
void listenToSecretKnock(){
  ...
  if (programModeActive == false){           // Only do this if we're not recording a new knock.
    if (validateKnock() == true){
      // Lock/unlock door
      chirp(500, 800);                  // And play a tone in case the user can't see the LED.
      chirp(500, 1500);
